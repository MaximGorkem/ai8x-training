...
def check_constraint(new_sample, constraint):
    ...
    if 'min_num_weights' in constraint:
        if new_sample.get_num_weights() < constraint['min_num_weights']:
            return False
    return True

def calc_accuracy(child_net_arch, mofa_base, train_loader, test_loader, device):
    correct = 0
    total = 0
    with torch.no_grad():
        subnet = update_arch(mofa_base, child_net_arch.get_param_dict())
...
class EvolutionSearch:
    ...
    def set_model(self, model):
        self.model = model
        self.arch = MOFA_Net_Arch(self.model.param_dict)
    ...
    def crossover_valid_sample(self, sample1, sample2, constraint):
        is_sample_ok = False
        while not is_sample_ok:
            new_sample = MOFA_Net_Arch.crossover(sample1, sample2)
            is_sample_ok = check_constraint(new_sample, constraint)
    ...
    def run(self, constraint, train_loader, test_loader, device):
        ...
        for _ in range(self.population_size):
            ...
            child_net_acc = calc_accuracy(child_net, self.model, train_loader, test_loader, device)
