...
def update_mofa_from_subnet_depth(mofa):
    ...
    for u_ind, unit in enumerate(mofa.units):
        ...
        param_dict['width_list'][u_ind] = [None] * len(unit.width_list)
        param_dict['bias_list'][u_ind] = [None] * len(unit.bias_list)
        for l_ind, layer in enumerate(unit.layers):
            param_dict['width_list'][u_ind][l_ind] = layer.conv2d.out_channels
            param_dict['bias_list'][u_ind][l_ind] = layer.conv2d.bias is not None 
    return mofa
...
## Elastic Kernel Training
n_units = 5

model = torch.load('mofa_models/arch_1/over4_clamp_mofa_acc64%.pth.tar')
...
for epoch in range(max_epochs):
    ...
    if val_accuracy > best_val_accuracy:
        if epoch != 0:
            os.remove(f'mofa_models/arch_1/Best_EK_over4_clamp_mofa_acc{100*best_val_accuracy:.0f}%_BN.pth.tar')
        torch.save(mofa, f'mofa_models/arch_1/Best_EK_over4_clamp_mofa_acc{100*val_accuracy:.0f}%_BN.pth.tar')
    ...
    if epoch % 200 == 0:
        torch.save(mofa, f'mofa_models/arch_1/EK_over4_clamp_mofa_acc{100*val_accuracy:.0f}%_ep{epoch}_BN.pth.tar')