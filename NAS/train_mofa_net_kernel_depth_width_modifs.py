...
for epoch in range(init_epochs, init_epochs+max_epochs):
    ...
    if val_accuracy > best_val_accuracy:
        if epoch != init_epochs:
            os.remove(f'mofa_models/mofa_ws_acc{100*best_val_accuracy:.0f}%.pth.tar')
        torch.save(mofa_base, f'mofa_models/mofa_ws_acc{100*val_accuracy:.0f}%.pth.tar')
        best_val_accuracy = val_accuracy
    print('\tAccuracy of the mofa on the test images: %d %%' % (100 * correct / total))

    if ((epoch+1) % 500) == 0:
        torch.save(mofa_base, f'mofa_models/mofa_ds_ep{epoch+1}_acc{100*val_accuracy:.0f}%.pth.tar')

torch.save(mofa_base, f'mofa_models/mofa_ws_acc{100*val_accuracy:.0f}%_epoch{epoch+1}.pth.tar')
